...
VisualFx::VisualFx(const zenkit::IEffectBase& fx, zenkit::DaedalusVm& vm, std::string_view name) {
  ...
  for(size_t i=0; i<zenkit::IEffectBase::user_string_count; ++i)
  ...
  for(int i=0; i<int(SpellFxKey::Count); ++i) {
    ...
    if(id==nullptr)
      continue;
    auto key = vm.init_instance<zenkit::IParticleEffectEmitKey>(id);
    keys  [i] = Key(*key);
    hasKey[i] = true;
    }
