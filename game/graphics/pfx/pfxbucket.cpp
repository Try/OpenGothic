...
PfxBucket::PfxBucket(const ParticleFx &decl, PfxObjects& parent, const SceneGlobals& scene, VisualObjects& visual)
  ...
  for(size_t i=0; i<Resources::MaxFramesInFlight; ++i) {
    ...
    item.pMain   = Shaders::inst().materialPipeline(decl.visMaterial, DrawCommands::Pfx, Shaders::T_Main,   Shaders::F_None);
    item.pShadow = Shaders::inst().materialPipeline(decl.visMaterial, DrawCommands::Pfx, Shaders::T_Shadow, Shaders::F_None);
  ...
  if(decl.hasTrails()) {
    ...
    for(size_t i=0; i<Resources::MaxFramesInFlight; ++i) {
      ...
      item.pMain   = Shaders::inst().materialPipeline(mat, DrawCommands::Pfx, Shaders::T_Main,   Shaders::F_None);
      item.pShadow = Shaders::inst().materialPipeline(mat, DrawCommands::Pfx, Shaders::T_Shadow, Shaders::F_None);
