...
Material::Material(const phoenix::material& m, bool enableAlphaTest) {
  ...
  if(m.texture_anim_map_mode!=0 && tex!=nullptr) {
...
Material::Material(const phoenix::vobs::vob& vob) {
  ...
  alpha        = loadAlphaFunc(vob.visual_decal->alpha_func,phoenix::material_group::undef,tex,true);
...
void Material::loadFrames(const phoenix::material& m) {
  ...
  if (m.texture_anim_fps > 0)
    texAniFPSInv = uint64_t(1000.f / m.texture_anim_fps);
  else
    texAniFPSInv = 1;
}
