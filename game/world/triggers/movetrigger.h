...
class MoveTrigger : public AbstractTrigger {
  ...
  private:
    ...
    void     advanceAnim(float a);
    float    advanceKey();
    uint32_t nextFrame(uint32_t i) const;
    uint32_t prevFrame(uint32_t i) const;
    float    applyMotionScaling(uint32_t f1, uint32_t f2, float a) const;
    auto     calculateTransform(uint32_t f1, uint32_t f2, float a) const -> Tempest::Matrix4x4;
