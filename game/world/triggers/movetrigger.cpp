...
void MoveTrigger::tick(uint64_t dt) {
 ...
 void MoveTrigger::advanceAnim() {
  ...
  float    a   = calculateProgress(f1,f2);
 ...
 bool MoveTrigger::isTargetFrame() {
  uint32_t f = (state==Close) ? prevFrame(frame) : frame;
  if(time<keyframes[f].ticks)
    return false;
  if(state==Close)
...
float MoveTrigger::calculateProgress(uint32_t f1, uint32_t f2) const {
  float a = float(time)/float(keyframes[f1].ticks);
  if(state==Close)
    a = 1 - a;
