...
MainWindow::MainWindow(Device& device)
  : Window(Maximized), device(device), swapchain(device, hwnd()), atlas(device), renderer(swapchain),
    rootMenu(keycodec), inventory(keycodec), dialogs(inventory), document(keycodec), console(*this),
  ...
  if(Gothic::inst().version().game == 2)
    setWindowTitle("Gothic II");
  else
    setWindowTitle("Gothic");

  if(!CommandLine::inst().isWindowMode()) setFullscreen(true);
...
void MainWindow::paintEvent(PaintEvent& event) {
  ...
  if(world == nullptr && !background.isEmpty()) {
    ...
    if(st == Gothic::LoadState::Idle) {
      ...
      p.drawRect(0, 0, w(), h(), 0, 0, background.w(), background.h());
    }
  }
  ...
  if(st != Gothic::LoadState::Idle && st != Gothic::LoadState::Finalize) {
    if(st == Gothic::LoadState::Saving) {
      ...
      if(auto back = Gothic::inst().loadingBanner()) {
        ...
        p.drawRect(0, 0, this->w(), this->h(), 0, 0, back->w(), back->h());
      }
    ...
    if(world != nullptr && world->view()) {
      ...
      if(auto pl = Gothic::inst().player()) {
        if(!Gothic::inst().isDesktop()) {
          ...
          if(showHealthBar) drawBar(p, barHp, 10, h() - 10, hp, AlignLeft | AlignBottom);
          if(showManaBar) drawBar(p, barMana, w() - 10, h() - 10, mp, AlignRight | AlignBottom);
          if(showSwimBar) {
...
void MainWindow::resizeEvent(SizeEvent&) {
  ...
  swapchain.reset();
  renderer.resetSwapchain();
  if(auto camera = Gothic::inst().camera()) camera->setViewport(swapchain.w(), swapchain.h());
...
void MainWindow::mouseDownEvent(MouseEvent& event) {
  if(event.button < sizeof(mouseP)) mouseP[event.button] = true;
...
void MainWindow::mouseUpEvent(MouseEvent& event) {
  ...
  if(event.button < sizeof(mouseP)) mouseP[event.button] = false;
}

void MainWindow::mouseDragEvent(MouseEvent& event) {
  const bool fs = SystemApi::isFullscreen(hwnd());
  if(!mouseP[Event::ButtonLeft] && !fs) return;
  if(player.focus().npc && !fs) return;
...
void MainWindow::tickMouse() {
  ...
  dpScaled *= 1000.f;
  dpScaled.y /= 7.f;
  if(camLookaroundInverse) dpScaled.y *= -1.f;
...
void MainWindow::onSettings() {
  ...
  if(zMaxFps <= 0) zMaxFps = Gothic::inst().settingsGetI("ENGINE", "zMaxFps");
  if(zMaxFps > 0)
    maxFpsInv = 1000u / uint64_t(zMaxFps);
  else
    maxFpsInv = 0;
}

void MainWindow::mouseWheelEvent(MouseEvent& event) {
  if(auto camera = Gothic::inst().camera()) camera->changeZoom(event.delta);
}
...
void MainWindow::keyUpEvent(KeyEvent& event) {
  ...
  if(!menuEv.empty()) {
    ...
    if(auto pl = Gothic::inst().player()) rootMenu.setPlayer(*pl);
    clearInput();
    ...
    if(inventory.isActive()) {
      ...
      if(pl != nullptr) inventory.open(*pl);
    }
    clearInput();
  }
...
void MainWindow::paintFocus(Painter& p, const Focus& focus, const Matrix4x4& vp) {
  if(!focus || dialogs.isActive()) return;
  ...
  if(focus.npc != nullptr && (foc == 1 || foc == 3) && player.isPressed(KeyCodec::ActionGeneric) &&
     pl->weaponState() != WeaponState::NoWeapon && pl->weaponState() != WeaponState::Fist) {
    auto tr = vp;
    tr.mul(focus.npc->transform());

    auto b    = focus.npc->bounds();
    Vec3 bx[] = {
        {b.bbox[0].x, b.bbox[0].y, b.bbox[0].z}, {b.bbox[1].x, b.bbox[0].y, b.bbox[0].z},
        {b.bbox[1].x, b.bbox[1].y, b.bbox[0].z}, {b.bbox[0].x, b.bbox[1].y, b.bbox[0].z},
        {b.bbox[0].x, b.bbox[0].y, b.bbox[1].z}, {b.bbox[1].x, b.bbox[0].y, b.bbox[1].z},
        {b.bbox[1].x, b.bbox[1].y, b.bbox[1].z}, {b.bbox[0].x, b.bbox[1].y, b.bbox[1].z},
    };
...
void MainWindow::drawBar(Painter& p, const Tempest::Texture2d* bar, int x, int y, float v, AlignFlag flg) {
  if(barBack == nullptr || bar == nullptr) return;
  const float scale   = Gothic::interfaceScale(this);
  ...
  if(flg & AlignRight)
    ...
    x -= int(destW) / 2;
  if(flg & AlignBottom) y -= int(destH);
  ...
  float pd = 9.f * k;
  p.setBrush(*bar);
  p.drawRect(x + int(pd), y + dy, int(float(destW - pd * 2) * v), int(destHin), 0, 0, bar->w(), bar->h());
}
...
void MainWindow::drawProgress(Painter& p, int x, int y, int w, int h, float v) {
  ...
  p.setBrush(*loadVal);
  p.drawRect(x + paddL, y + paddT, int(float(w - 2 * paddL) * v), h - 2 * paddT, 0, 0, loadVal->w(), loadVal->h());
}
...
void MainWindow::drawSaving(Painter& p) {
  ...
  if(saveback == nullptr) saveback = Resources::loadTexture("SAVING.TGA");
  if(saveback == nullptr) return;
...
void MainWindow::drawSaving(Painter& p, const Tempest::Texture2d& back, int sw, int sh, float scale) {
  ...
  float v = float(Gothic::inst().loadingProgress()) / 100.f;
  drawProgress(p, x + int(100.f * scale), y + sh - int(75.f * scale), sw - 2 * int(100.f * scale), int(40.f * scale),
               v);
}
  ...
  switch(k) {
    ...
    case Event::K_F6:
      if(Gothic::inst().isMarvinEnabled() && !dialogs.isActive()) {
        ...
        if(camera != nullptr && inter == nullptr) camera->setMarvinMode(Camera::M_Free);
      }
      break;
    case Event::K_F7:
      if(Gothic::inst().isMarvinEnabled() && !dialogs.isActive()) {
    ...
    case Event::K_F10:
      if(runtimeMode == R_Suspended) runtimeMode = R_Step;
      break;
    case Event::K_P:
      if(Gothic::inst().isMarvinEnabled()) {
...
uint64_t MainWindow::tick() {
  ...
  if(st == Gothic::LoadState::Finalize || st == Gothic::LoadState::FailedLoad || st == Gothic::LoadState::FailedSave) {
    ...
    if(st == Gothic::LoadState::FailedLoad) rootMenu.setMainMenu();
    if(st == Gothic::LoadState::FailedSave) Gothic::inst().onPrint("unable to write savegame file");
    return 0;
  } else if(st != Gothic::LoadState::Idle) {
    if(st == Gothic::LoadState::Loading)
      GameMusic::inst().setMusic(GameMusic::SysLoading);
    else
      rootMenu.processMusicTheme();
    return 0;
  }
  ...
  if(dialogs.isActive())
    ; // clearInput();
  if(document.isActive()) clearInput();
  tickMouse();
  player.tickMove(dt);
  update();
  return dt;
...
void MainWindow::tickCamera(uint64_t dt) {
  ...
  const bool meleeFocus = (ws == WeaponState::Fist || ws == WeaponState::W1H || ws == WeaponState::W2H);
  ...
  if(!camera.isCutscene()) {
    ...
    if(!fs && mouseP[Event::ButtonLeft]) {
      ...
      auto spin = camera.destSpin();
      if(pl->interactive() == nullptr && !pl->isDown()) spin.y = pl->rotation();
      if(pl->isDive() && !camera.isMarvin()) spin.x = -pl->rotationY();
      camera.setDestSpin(spin);
      camera.setDestPosition(pos);
    }
  }
  ...
  if(dt == 0) return;
  if(camera.isToggleEnabled() && !camera.isCutscene()) camera.setMode(solveCameraMode());
  camera.tick(dt);
}

Camera::Mode MainWindow::solveCameraMode() const {
  if(auto camera = Gothic::inst().camera()) {
    if(camera->isFree()) return Camera::Normal;
  }

  if(inventory.isOpen() == InventoryMenu::State::Equip || inventory.isOpen() == InventoryMenu::State::Ransack)
    return Camera::Inventory;

  if(auto pl = Gothic::inst().player()) {
    if(pl->interactive() != nullptr) return Camera::Mobsi;
  }

  if(dialogs.isActive()) return Camera::Dialog;

  if(auto pl = Gothic::inst().player()) {
    if(pl->isDead()) return Camera::Death;
    if(pl->isDive()) return Camera::Dive;
    if(pl->isSwim()) return Camera::Swim;
    if(pl->isFallingDeep()) return Camera::Fall;
...
void MainWindow::saveGame(std::string_view slot, std::string_view name) {
  ...
  if(dialogs.isActive()) return;
  if(auto w = Gothic::inst().world(); w != nullptr && w->currentCs() != nullptr) return;

  Gothic::inst().startSave(std::move(textureCast<Texture2d&>(tex)), [slot = std::string(slot), name = std::string(name),
                                                                     pm](std::unique_ptr<GameSession>&& game) {
    if(!game) return std::move(game);
...
void MainWindow::onVideo(std::string_view fname) {
  if(Gothic::inst().isBenchmarkMode()) return;
...
void MainWindow::onWorldLoaded() {
  ...
  if(auto c = Gothic::inst().camera()) c->setViewport(uint32_t(w()), uint32_t(h()));

  renderer.onWorldChanged();

  if(auto pl = Gothic::inst().player()) pl->multSpeed(1.f);
...
void MainWindow::onBenchmarkFinished() {
  if(benchmark.numFrames == 0) return;
...
void MainWindow::render() {
  try {
    ...
    fps.push(t - time);
    if(Gothic::inst().isBenchmarkMode() && Gothic::inst().world() != nullptr &&
       Gothic::inst().world()->currentCs() != nullptr)
